...
Shader "Amplify Scripted Sandstone Shader"
{
	Properties
	{
		...
		_AmbientOcclusionRemapping("Ambient Occlusion Remapping", Range( 0 , 1)) = 0.5
		_DetailNormals("Detail Normals", 2D) = "white" {}
		_DetailOpacityOverride("Detail Opacity Override", Range( 0 , 1)) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _MaskMap_ST;
			float _DetailOpacityOverride;
			float _AmbientOcclusionRemapping;
						...
						)
			{
				...
				float2 uv_BaseMap = packedInput.ase_texcoord6.xy * _BaseMap_ST.xy + _BaseMap_ST.zw;
				...
				float2 uv_DetailMap = packedInput.ase_texcoord6.xy * _DetailMap_ST.xy + _DetailMap_ST.zw;
				float temp_output_102_0 = ( GlobalAlbedoOpacity * _DetailOpacityOverride );
				float4 temp_cast_0 = ((saturate( ( -temp_output_102_0 + 0.5 ) ) + (tex2D( _DetailMap, uv_DetailMap ).r - 0.0) * (saturate( ( temp_output_102_0 + 0.5 ) ) - saturate( ( -temp_output_102_0 + 0.5 ) )) / (1.0 - 0.0))).xxxx;
				float4 blendOpSrc80 = ( saturate( 2.0f*blendOpDest12*blendOpSrc12 + blendOpDest12*blendOpDest12*(1.0f - 2.0f*blendOpSrc12) ));
				float4 blendOpDest80 = temp_cast_0;
				
				float2 uv_NormalMap = packedInput.ase_texcoord6.xy * _NormalMap_ST.xy + _NormalMap_ST.zw;
				float2 uv_DetailNormals = packedInput.ase_texcoord6.xy * _DetailNormals_ST.xy + _DetailNormals_ST.zw;
				float2 uv_MaskMap = packedInput.ase_texcoord6.xy * _MaskMap_ST.xy + _MaskMap_ST.zw;
				float4 tex2DNode3 = tex2D( _MaskMap, uv_MaskMap );
				float3 unpack106 = UnpackNormalScale( tex2D( _DetailNormals, uv_DetailNormals ), ( tex2DNode3.b * DetailNormalStrength ) );
				unpack106.z = lerp( 1, unpack106.z, saturate(( tex2DNode3.b * DetailNormalStrength )) );
				
				surfaceDescription.BaseColor = ( saturate( (( blendOpDest80 > 0.5 ) ? ( 1.0 - 2.0 * ( 1.0 - blendOpDest80 ) * ( 1.0 - blendOpSrc80 ) ) : ( 2.0 * blendOpDest80 * blendOpSrc80 ) ) )).rgb;
				surfaceDescription.Normal = BlendNormal( UnpackNormalScale( tex2D( _NormalMap, uv_NormalMap ), 1.0f ) , unpack106 );
				...
				surfaceDescription.Smoothness = ( tex2DNode3.a * GlobalSmooth );
				surfaceDescription.Occlusion = saturate( ( tex2DNode3.b + _AmbientOcclusionRemapping ) );
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float4 blendOpSrc12 = tex2DNode1;
				float4 blendOpDest12 = ( GlobalTint * saturate( ( GlobalContrast + tex2DNode1.r ) ) );
				float2 uv_DetailMap = packedInput.ase_texcoord2.xy * _DetailMap_ST.xy + _DetailMap_ST.zw;
				float temp_output_102_0 = ( GlobalAlbedoOpacity * _DetailOpacityOverride );
				float4 temp_cast_0 = ((saturate( ( -temp_output_102_0 + 0.5 ) ) + (tex2D( _DetailMap, uv_DetailMap ).r - 0.0) * (saturate( ( temp_output_102_0 + 0.5 ) ) - saturate( ( -temp_output_102_0 + 0.5 ) )) / (1.0 - 0.0))).xxxx;
				float4 blendOpSrc80 = ( saturate( 2.0f*blendOpDest12*blendOpSrc12 + blendOpDest12*blendOpDest12*(1.0f - 2.0f*blendOpSrc12) ));
				float4 blendOpDest80 = temp_cast_0;
		...
		Pass
		{
					...
					)
			{
				...
				float2 uv_DetailNormals = packedInput.ase_texcoord4.xy * _DetailNormals_ST.xy + _DetailNormals_ST.zw;
				float2 uv_MaskMap = packedInput.ase_texcoord4.xy * _MaskMap_ST.xy + _MaskMap_ST.zw;
				float4 tex2DNode3 = tex2D( _MaskMap, uv_MaskMap );
				float3 unpack106 = UnpackNormalScale( tex2D( _DetailNormals, uv_DetailNormals ), ( tex2DNode3.b * DetailNormalStrength ) );
				unpack106.z = lerp( 1, unpack106.z, saturate(( tex2DNode3.b * DetailNormalStrength )) );
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 uv_BaseMap = packedInput.ase_texcoord8.xy * _BaseMap_ST.xy + _BaseMap_ST.zw;
				float4 tex2DNode1 = tex2D( _BaseMap, uv_BaseMap );
				float4 blendOpSrc12 = tex2DNode1;
				float4 blendOpDest12 = ( GlobalTint * saturate( ( GlobalContrast + tex2DNode1.r ) ) );
				float2 uv_DetailMap = packedInput.ase_texcoord8.xy * _DetailMap_ST.xy + _DetailMap_ST.zw;
				float temp_output_102_0 = ( GlobalAlbedoOpacity * _DetailOpacityOverride );
				float4 temp_cast_0 = ((saturate( ( -temp_output_102_0 + 0.5 ) ) + (tex2D( _DetailMap, uv_DetailMap ).r - 0.0) * (saturate( ( temp_output_102_0 + 0.5 ) ) - saturate( ( -temp_output_102_0 + 0.5 ) )) / (1.0 - 0.0))).xxxx;
				float4 blendOpSrc80 = ( saturate( 2.0f*blendOpDest12*blendOpSrc12 + blendOpDest12*blendOpDest12*(1.0f - 2.0f*blendOpSrc12) ));
				float4 blendOpDest80 = temp_cast_0;
				...
				float2 uv_DetailNormals = packedInput.ase_texcoord8.xy * _DetailNormals_ST.xy + _DetailNormals_ST.zw;
...
WireConnection;26;1;107;0
WireConnection;26;7;10;0
WireConnection;26;8;25;0
ASEEND*/
//CHKSM=A5BBFA4936F91ED2AED7B6880CE5C4341ABF733D