...
Shader "Amplify Scripted Tile Shader"
{
	Properties
	{
		...
		_Tiling("Tiling", Range( 0 , 0.15)) = 0.1
		_GlobalSmoothOverride("Global Smooth Override", Float) = 1
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float2 appendResult29 = (float2(_OffsetX , _OffsetY));
				float2 temp_output_31_0 = ( ( (ase_positionWS).xz + appendResult29 ) * _Tiling );
				float4 tex2DNode5 = tex2D( _BaseMap, temp_output_31_0 );
				float4 blendOpSrc1 = tex2DNode5;
				float4 blendOpDest1 = ( GlobalTint * saturate( ( GlobalContrast + tex2DNode5.r ) ) );
				
				float3 unpack8 = UnpackNormalScale( tex2D( _NormalMap, temp_output_31_0 ), _NormalStr );
				unpack8.z = lerp( 1, unpack8.z, saturate(_NormalStr) );
				...
				float3 worldToTangentDir38 = mul( ase_worldToTangent, (unpack8).xzy);
				
				float4 tex2DNode9 = tex2D( _MaskMap, temp_output_31_0 );
				
				surfaceDescription.BaseColor = ( saturate( 2.0f*blendOpDest1*blendOpSrc1 + blendOpDest1*blendOpDest1*(1.0f - 2.0f*blendOpSrc1) )).rgb;
				surfaceDescription.Normal = worldToTangentDir38;
				...
				surfaceDescription.Smoothness = ( tex2DNode9.a * GlobalSmooth * _GlobalSmoothOverride );
